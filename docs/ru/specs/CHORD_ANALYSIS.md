...
**Data Layer**
- **`ChordAnalysisRepositoryImpl`**: содержит `MutableStateFlow<String?>` и публичный метод `analyzeChord(notes: List<Note>)`. Упорядочивает ноты по высоте (по возрастанию, чтобы самая низкая нота была басом), дедуплицирует имена нот, затем синхронно вызывает `ChordAnalyzer.analyze(...)` и записывает результат напрямую в `StateFlow`. Без многопоточности и платформенных зависимостей.
...
class ChordAnalyzer @Inject constructor() {
     ...
     *
     * @param noteNames Упорядоченный по высоте (бас первым) список названий нот без дубликатов (например, ["C4", "E4", "G4"]).
     *     Первый элемент трактуется как басовая нота.
...
| `["C#4", "Db4"]` (разные написания, одинаковый chroma) | первое написание побеждает (порядок входа сохраняется) → только один pitch class, аккорд не распознается → `null` |
| `["C4", "D4", "E4"]` (нет совпадающего типа аккорда) | `null` |

**Басовая нота.** Репозиторий упорядочивает ноты по высоте (по возрастанию) до вызова анализатора, так что самая низкая звучащая нота идет первой и трактуется как бас. Это необходимо для корректного распознавания slash-аккордов и аккордов с тоникой — лексикографическая сортировка по имени поставила бы ноты вроде `A#`/`B` (которые алфавитно идут перед `C`) впереди настоящего баса.

**Энгармоническая стабильность.** Таблица chroma → имя строится в порядке поступления входов. Для MIDI-входа каждое `Note.name` детерминированно выводится из высоты (диезное написание), поэтому два разных написания одного pitch class не доходят до анализатора из репозитория; а если бы дошли — порядок по высоте (затем по входу) определил бы, какое написание побеждает.
...
4. `ChordAnalysisRepositoryImpl` упорядочивает ноты по высоте (бас первым) и дедуплицирует имена нот, затем синхронно вызывает `ChordAnalyzer.analyze(noteNames)`.
...
- Эталонные значения `chordAnalysisResult`:
  - `[C4, E4, G4]` → `"C"`
  - `[A4, C5, E5]` → `"Am"`
  - `[A3, D4, F4]` → `"Dm/A"` (A — бас, но корень D; slash-аккорд через реальный поток)
  - `[C4, D4, E4]` → `null`
