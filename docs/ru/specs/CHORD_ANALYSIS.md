...
**Domain Layer**
- **`Pitch`** (`domain.model`): разобранная нота — буква, альтерация, октава (nullable), MIDI-номер (nullable), chroma. Чистая data-модель.
...
**Жесткая фиксация инвариантов.** `Pitch` хранит только каноническую тройку (`letter`, `alter`, `octave`); `chroma` и `midi` — производные свойства, вычисляемые из неё детерминированно, поэтому экземпляр с рассогласованными звучащими значениями на уровне типа невозможен. Допустимость `letter ∈ A..G` обеспечивается типом — это enum `NoteLetter` с семью записями (`C`, `D`, `E`, `F`, `G`, `A`, `B`); параметр `alter` проверяется в `init`-блоке на принадлежность диапазону `-2..2`. Вычисление `midi` использует арифметику `Long`, чтобы защититься от переполнения `Int` для произвольных значений `octave`, допускаемых парсером.
