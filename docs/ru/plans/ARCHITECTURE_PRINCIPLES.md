...
## 6. Система UI/UX-дизайна

### 6.1. Material Design 3

Приложение следует руководству **[Material Design 3](https://m3.material.io/)** как единому стандарту UI/UX для всех визуальных компонентов и паттернов взаимодействия.

**Ключевые принципы M3, применяемые в проекте**:
- **Цветовая система**: динамические цветовые роли (`Primary`, `Secondary`, `Tertiary`, `Surface`, `Error` и их варианты `On*`/`Container`). Все цвета задаются через `MaterialTheme.colorScheme` — хардкод цветов в composable-функциях запрещен.
- **Типография**: шкала M3 (`displayLarge` → `labelSmall`). Все стили текста берутся из `MaterialTheme.typography`.
- **Форма**: система фигур M3 (`extraSmall` → `extraLarge`). Радиусы скругления берутся из `MaterialTheme.shapes`.
- **Компоненты**: предпочтение M3 Compose-компонентам (`Button`, `Card`, `TopAppBar`, `NavigationBar`, `Scaffold` и др.) перед кастомными реализациями там, где M3 предоставляет эквивалент.
- **Темизация**: тема приложения определяется в слое `presentation` и передается через корневой composable. Поддержка динамических цветов (Android 12+) опциональна; статическая цветовая палетка-фоллбек всегда присутствует.

**Ссылки**:
- Спецификация Material Design 3: [m3.material.io](https://m3.material.io/)
...
- Material Theme Builder (инструмент для палетки): [material-foundation.github.io/material-theme-builder](https://material-foundation.github.io/material-theme-builder)

## 7. Общая структура пакетов
...
## 9. Оптимизация сборки (R8/ProGuard)
...
## 11. Вынесение ядра как отдельной библиотеки
...
### 11.1. Структура для мультиплатформенного использования
...
### 11.2. Адаптеры для разных платформ
...
### 11.3. Схема C4: Контекст и контейнеры
...
## 12. Строковые ресурсы и локализация

### 12.1. Принципы работы со строками
