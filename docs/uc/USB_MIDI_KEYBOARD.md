...
6. Система отображает уведомление с текстом "MIDI-клавиатура подключена"
...
5. Система отображает уведомление с текстом "MIDI-клавиатура подключена"
...
7. Система отображает уведомление с сообщением об ошибке
...
5. Система отображает уведомление о подключении первого устройства
6. Система продолжает отслеживать состояние подключения
...
3. Система отображает уведомление с текстом "Нет разрешения на доступ к MIDI-устройству"

#### E2: MIDI API недоступен
1. Система не может использовать MIDI API (не поддерживается устройством)
2. Система обрабатывает ошибку через UC-002
3. Система отображает уведомление с текстом "MIDI не поддерживается на данном устройстве"
...
### Краткое описание
Система обрабатывает различные типы ошибок, возникающих при работе с MIDI-клавиатурой, и преобразует их в понятные сообщения для пользователя, отображаемые через уведомления.
...
2. Пользователь информирован об ошибке через уведомление
...
5. Система отображает уведомление с сообщением об ошибке
...
4. Система отображает уведомление с этим сообщением
...
5. Система отображает уведомление с этим сообщением
...
loop Отслеживание подключений
  alt Клавиатура подключена
    ...
    Tracker -> UI : Устройство подключено
    ...
    Tracker -> UI : Устройство отключено
    UI -> UI : Скрывает уведомления\n(ничего не показывает)
  end
end
