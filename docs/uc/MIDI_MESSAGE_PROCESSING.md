...
-   **Отображение одиночных нот**: При нажатии одной клавиши на экране появляется ее MIDI-номер.
-   **Отображение аккордов**: При одновременном нажатии нескольких клавиш на экране отображаются MIDI-номера всех нот аккорда.
-   **Обновление экрана**: Перед отображением новых нот, экран очищается от предыдущих.
-   **Прототип экрана**: Для проверки сквозной работы функционала, на данном этапе реализации экран представляет собой текстовое поле, отображающее MIDI-номера сыгранных нот. Полноценная отрисовка на нотном стане является целью будущих доработок.
...
### Краткое описание
Система принимает MIDI-сообщения от подключенной клавиатуры, обрабатывает события нажатия клавиш и отображает на экране текстовую информацию о сыгранных нотах или аккордах (их MIDI-номера).
...
### Постусловия
1.  Пользователь видит на экране MIDI-номера нот, соответствующие нажатым на клавиатуре клавишам.
...
3.  Система очищает экран от всех ранее отображенных номеров нот.
4.  Система обрабатывает сообщение, извлекая MIDI-номер ноты.
5.  Система отображает MIDI-номер ноты на экране в виде текста.
...
5.  Система отображает все MIDI-номера аккорда на экране в виде текста.
6.  Система ожидает следующего события нажатия.
...
alt MIDI-сообщение == "Note On" (нажатие клавиши или аккорд)
    ...
    UI -> UI : Очищает экран
    UI -> UI : Отображает текстовую информацию о нотах
    UI -> User : Показывает "Сыграны ноты: 60, 64, 67"
...
1.  При нажатии одной клавиши на MIDI-клавиатуре соответствующая нота немедленно отображается на экране (в виде MIDI-номера).
2.  При одновременном нажатии нескольких клавиш (аккорд) все соответствующие ноты отображаются на экране (в виде MIDI-номеров).
