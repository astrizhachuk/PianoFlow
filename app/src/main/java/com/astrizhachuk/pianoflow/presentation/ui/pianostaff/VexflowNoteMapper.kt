...
/**
 * Parses a VexFlow JSON string to extract a unique, sorted list of note names formatted for chord analysis.
 *
 ...
 * 3.  **Formatting:** It converts VexFlow's "c#/4" format to the "C#4" format used by the
 *     chord analysis functions by capitalizing the note letter.
 ...
 * @return A distinct, sorted list of note names in the format required for chord analysis (e.g., `["C5", "E3", "G3"]`).
...
/**
 * Processes the raw result from the JavaScript chord analysis to return a clean and standardized chord name.
 *
 * This function performs several key tasks:
 * 1.  **Cleaning:** It removes surrounding quotes and handles empty or "null" string values
 *     returned from the WebView.
 * 2.  **Standardization:** The chord analysis may return major chords with an "M" suffix
...
fun processChordAnalysisResult(
    ...
    return when {
        // The chord analysis may return "CM" for C-Major, so we remove the "M" for consistency.
